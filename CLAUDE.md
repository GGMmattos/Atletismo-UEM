...
Materiais de origem (não fazem parte do site em si): tudo dentro de `/materiais-origem` — `detalhamento.pdf` (proposta original), `ranking-atletismo-uem.xlsx` (fonte da migração única do ranking, não é reimportada depois), `imagens/` (logos e fotos originais não otimizados — as versões usadas pelo site já foram migradas para `/public/logos` e `/public/galeria`), `noticias/` (rascunhos `.docx` recebidos da coordenação — cada notícia publicada tem seu conteúdo transcrito para `content/noticias/<slug>/index.md`, com as fotos correspondentes em `public/noticias/<slug>/`; o site nunca lê `.docx` diretamente), `atletas/` (biografias em `.docx` e fotos originais por atleta, antes de virarem entradas em `data/atletas.json` e fotos otimizadas em `/public/atletas`), `artigos/` (PDFs dos artigos científicos referenciados em `data/artigos.json`), `formularios/` e `info-projeto/`. Ver `materiais-origem/README.md` para o detalhamento de cada subpasta.
...
**Dados e conteúdo**: `/data/*.json` (ranking, atletas, impacto, resultados, galeria, equipe-tecnica, parceiros) são importados diretamente nos `page.tsx` (Server Components) via `resolveJsonModule` e passados como props — sem fetch em runtime. `/content/*.md` (textos institucionais) usam frontmatter + corpo Markdown, lidos em build-time por `lib/content.ts` (`gray-matter` + `remark`). Campos opcionais ausentes (ex: `data`/`competicao` no ranking, `impacto.json`) são `null` no JSON — os componentes tratam isso explicitamente para nunca renderizar a string `"null"` (ex: `RankingTable`, `ImpactoStats`). Exceção: `atletasNaEquipe` não existe em `data/impacto.json` — é calculado em `app/page.tsx` como `atletas.length` a partir de `data/atletas.json`, para não exigir atualização manual sempre que um atleta é adicionado/removido. **Notícias são a exceção de layout**: cada notícia é uma pasta, não um arquivo solto — `content/noticias/<slug>/index.md` (texto) com as fotos correspondentes em `public/noticias/<slug>/` (mesmo `slug` nos dois lugares); `getAllNoticias`/`getNoticia` em `lib/content.ts` esperam esse formato de pasta. O frontmatter aceita um campo opcional `fotos` (lista de `{ src, alt }`), renderizado como carrossel pelo componente `NoticiaCarrossel` no fim da página da notícia — distinto de imagens `![...]()` soltas no corpo do Markdown.
