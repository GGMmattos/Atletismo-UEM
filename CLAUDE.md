...
**Tailwind v4**: não há `tailwind.config.ts` — a paleta da marca é definida via `@theme inline` em `app/globals.css` (`--color-uem-black`, `--color-uem-red`, `--color-uem-red-deep`, `--color-uem-white`). Dois tons de vermelho existem de propósito: `uem-red` (#EE2E3C, a cor de marca) falha contraste AA (4.5:1) como texto pequeno sobre fundo branco — use `uem-red` só em fundo escuro, texto grande (≥24px) ou elementos decorativos/bordas; use `uem-red-deep` (#B71C2B) em texto e fundos de botão sobre fundo claro. Não introduza um terceiro tom sem necessidade.
